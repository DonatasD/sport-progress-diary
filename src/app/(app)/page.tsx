...
import { formatDuration, startOfWeek } from "@/lib/utils";
      ...
      <section className="space-y-3">
        ...
        {sessions.length === 0 ? (
          ...
          <div className="space-y-3">
            {sessions.slice(0, 8).map((s) => <SessionCard key={s.id} session={s} />)}
          </div>
        )}
      </section>
    </div>
...
function weekStreak(sessions: SessionRow[]) {
  const weeks = new Set(sessions.map((s) => startOfWeek(new Date(s.performed_at)).getTime()));
